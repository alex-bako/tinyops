drop index if exists public.data_sources_one_active_per_workspace_type;

create unique index if not exists data_sources_one_active_per_workspace_singleton_type
...
create or replace function public.is_valid_google_forms_csv_mapping(mapping jsonb)
returns boolean
language sql
immutable
set search_path = ''
as $$
  select mapping is not null
    and jsonb_typeof(mapping) = 'object'
    and jsonb_typeof(mapping->'identityColumn') = 'string'
    and btrim(mapping->>'identityColumn') <> ''
    and jsonb_typeof(mapping->'timestampColumn') = 'string'
    and btrim(mapping->>'timestampColumn') <> ''
    and btrim(mapping->>'identityColumn') <> btrim(mapping->>'timestampColumn')
$$;
...
create table public.google_forms_csv_uploads (
  ...
  constraint google_forms_csv_uploads_file_name_present check (
    btrim(file_name) <> ''
  ),
  constraint google_forms_csv_uploads_row_count_nonnegative check (
    row_count >= 0
  )
);

alter table public.google_forms_csv_uploads enable row level security;

create trigger google_forms_csv_uploads_set_updated_at
  before update on public.google_forms_csv_uploads
  for each row
  execute function public.set_updated_at();

create index google_forms_csv_uploads_source_created_idx
  on public.google_forms_csv_uploads (source_id, created_at desc);

create policy "Members can read Google Forms CSV upload metadata"
  on public.google_forms_csv_uploads
  for select
  to authenticated
  using (public.workspace_actor_role(workspace_id) is not null);

create table public.google_forms_csv_rows (
  id uuid primary key default gen_random_uuid(),
  upload_id uuid not null references public.google_forms_csv_uploads(id) on delete cascade,
  source_id uuid not null references public.data_sources(id) on delete cascade,
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  ...
  created_at timestamptz not null default now(),
  constraint google_forms_csv_rows_row_number_positive check (row_number > 0),
  constraint google_forms_csv_rows_response_key_present check (
    btrim(response_key) <> ''
  ),
  constraint google_forms_csv_rows_payload_object check (
    jsonb_typeof(payload) = 'object'
  ),
  unique (source_id, response_key),
  unique (upload_id, row_number)
);

alter table public.google_forms_csv_rows enable row level security;

create index google_forms_csv_rows_upload_row_number_idx
  on public.google_forms_csv_rows (source_id, upload_id, row_number);

revoke all on public.google_forms_csv_rows from anon, authenticated;
grant select on public.google_forms_csv_rows to service_role;
grant select on public.google_forms_csv_uploads to authenticated, service_role;
grant insert, update on public.google_forms_csv_uploads to service_role;
grant insert, update, delete on public.google_forms_csv_rows to service_role;
