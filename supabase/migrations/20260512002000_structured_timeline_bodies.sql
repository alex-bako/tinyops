alter table public.timeline_events
  add column if not exists body jsonb not null default '{"text":"","blocks":[]}'::jsonb;
...
set body = jsonb_build_object(
  ...
  'blocks',
  case
    when btrim(coalesce(body_text, '')) = '' then '[]'::jsonb
    else jsonb_build_array(
      jsonb_build_object('kind', 'text', 'text', btrim(body_text))
    )
  end
)
where body = '{"text":"","blocks":[]}'::jsonb;
...
where event.source_id = source.id
  and event.event_type = 'form_submission'
  and not coalesce(event.metadata, '{}'::jsonb) ? 'formTitle';
...
alter table public.timeline_events
  add constraint timeline_events_body_shape check (
    public.is_valid_timeline_event_body(body)
  );
